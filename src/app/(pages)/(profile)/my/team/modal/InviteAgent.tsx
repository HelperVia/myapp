...
import { addAgent } from "@/store/agent/agent.slice";
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-[20px]">
      ...
      <div className="flex flex-row gap-4">
        ...
        <Button color="brandOrange" onClick={done} variant="contained">
          Done
        </Button>
      </div>
    </div>
...
export const InviteAgent = () => {
  ...
  const { modal, setModal } = React.use(ModalContext);
  const [roleValue, setRoleValue] = useState<string>(AGENT_ROLE_AGENT);
  ...
  const isButtonDisabled =
    !getValues("email") || getValues("email") === "" || isValidating || loading;
  ...
  const emailValidate = async (): Promise<boolean> => {
    ...
    try {
      ...
      const existsInRedux = agents?.some(
        (agent: AgentType) =>
          agent.email?.toLowerCase() === getValues("email").toLowerCase()
      );

      if (existsInRedux) {
        setError("email", {
          type: "manual",
          message: "The email address is already registered with your team",
        });
        setIsValidating(false);
        return false;
      }

      const data = await apiFetch<{ taken: boolean }>(
        "/api/team/agent/invite/validate/email",
        {
          method: "POST",
  ...
  const InviteAction = async () => {
    ...
    const isValid = await emailValidate();
    ...
    if (!getFieldState("email").error) {
      ...
      if (response?.ok) {
        dispatch(addAgent(response?.data?.agent));
        setLoading(false);
        setModal({
          open: true,
          size: "xs",
  ...
  return (
    <div className="flex flex-col justify-between gap-2">
      ...
      <div className="flex flex-row justify-end  gap-6 mt-12">
        <div>
          <Button
            loading={loading}
