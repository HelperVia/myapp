...
import { ChangeChatLimit } from "../../modal/change-change-limit";
...
export const AgentActionsRender = (props) => {
  ...
  const changeChatLimitModal = () => {
    ...
    setModal({
      ...
      fullScreen: false,
      title: false,
      content: <ChangeChatLimit agent={props.data} />,
    });
  };
  const deleteAction = () => {
    ...
    API.post("team/delete", formData).then(async (response) => {
      ...
      if (responseStatus) {
        setNotify({
          open: true,
          severity: "success",
          message: props.data.agent_name + " deleted",
        });
  ...
  const confirm = () => {
    ...
    setModal({
      ...
      content: (
        <ConfirmForm
          title={"'" + props.data.agent_name + "' agent will be deleted"}
          confirmComponent={
            <Button
        ...
        <HvPopoverContent close={closePopover}>
          <div className="hv-user-account-container w-[180px]">
            <ul className="m-0 pt-[6px] pb-[6px] pr-0 pl-0">
              <Link href={"/agents/" + props.data["email"]}>
              ...
              {props.data.role != AGENT_ROLE_OWNER && (
                <li
                  onClick={confirm}
                  className="gap-2 mt-0 mb-0 mr-[8px] ml-[8px] flex items-center hover:bg-bg-color-1 hover:text-hv-color-13 rounded-[8px] pt-[9px] pb-[9px] pl-[8px] pr-[8px] flex cursor-pointer"
                >
